...
public class ExactSearcher {
    ...
    public TopDocs searchLeaf(final LeafReaderContext leafReaderContext, final ExactSearcherContext context) throws IOException {
        ...
        return searchTopCandidates(context, iterator, context.getK(), Predicates.alwaysTrue());
    }
    ...
    private TopDocs searchTopCandidates(ExactSearcherContext context, KNNIterator iterator, int limit, @NonNull Predicate<Float> filterScore) throws IOException {
        int THRESHOLD = 250_000;
        int workers = (int) context.getNumberOfMatchedDocs() / THRESHOLD;
        List<Callable<TopDocs>> tasks = new ArrayList<>(workers);
        ...
        List<TopDocs> result = taskExecutor.invokeAll(tasks);
        return TopDocs.merge(context.getK(), result.toArray(new TopDocs[workers]));
    }
    ...
    private TopDocs filterDocsByMinScore(ExactSearcherContext context, KNNIterator iterator, float minScore) throws IOException {
        ...
        return searchTopCandidates(context, iterator, maxResultWindow, scoreGreaterThanOrEqualToMinScore);
    }
